    ...
    <div className="p-4 border-[1px] flex flex-col justify-between border-[#E0E1EA] rounded-md">
      <div className="flex flex-col gap-2">
        <div className="flex justify-between items-center">
          <p className="text-[20px] text-[#343432] font-medium">{title}</p>
          {icon && icon}
        </div>
        <p className="text-[24px] text-[#343432] font-medium">{amount}</p>
      </div>
      {ctaValues?.length > 0 && (
        <div className="flex bg-gray-200 mt-4 gap-3 rounded-md w-fit">
          {ctaValues?.map((item, index) => {
            return (
              ...
              >
                {item?.icon}
                <p className="text-[12px] text-[#343432] font-medium ">
                  {item?.name}
                </p>
              </div>
            );
      ...
      <div>
        {note1 && (
          <p className="text-[14px] text-[#343432] font-medium ">{note1}</p>
        )}
        {note && (
            ...
            {accountDetails?.map((item, index) => {
              return (
                <div key={index}>
                  ...
                  <p className="text-[13px] text-[#343432] font-medium ">
                    {item?.value}
                  </p>
                </div>
              );
